...
**⚠️ 2026-08-06: THIS IS NOW THE FIRST TASK, ahead of D2.** `iteration_budget = target_per_cat × mult` (`engine.rs:82-86`), so at `mult = 2` Studio 200 goes from 200 attempts (~26 min measured) to **400 attempts ≈ 52 min**. Every Studio number in this repo is provisional until this lands. Measuring first and changing the multiplier second means measuring twice.
...
**⚠️ 2026-08-06 — a first attempt was made (`5940dd2`) and it does not count. Re-take it.** Result was 200 requested → 124 delivered in 26.2 min, but: no CSV was written, so the number is unreproducible; no rejection outcomes were logged, so the stated cause is unmeasured; the distinctness rule was reimplemented weaker than the engine's own; and it is n=1. **Requirements for the re-take, all mandatory:**
