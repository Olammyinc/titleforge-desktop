...
**T1. Tier caps + tier-aware machine activations *(small, do first)* — ✅ DONE 2026-08-07.**
- `lib.rs:102-104` — `core => 50`, `pro => 100`, `studio => 200` (Core was 25, Pro 50). Desktop `27394f3`. `cargo check` + 33/33.
- `licenses.js:168` — `MAX_MACHINES` flat `3` → **tier-aware 1/3/5** via `maxDevices(tier)`, mirrored in the authoritative `register_machine` RPC (`supabase-setup.sql`, tier CASE) and both `max_devices` return fields. Web `6d4e3b7`. `node --check` + 28/28. (`desktop-download.html` reads the API's `max_devices`, so it's tier-aware automatically; the 409 hardcoded "3 machines" string was removed.)
- ~~⚠️ Existing licences already have up to 3 machines registered — grandfather, or clamp only on new registrations.~~ **NOT NEEDED — owner confirmed 2026-08-07 there are no beta testers and no licences in the wild.** Clamp cleanly on all validations; write no grandfathering logic. If the owner's own test keys trip the new Core limit, clear `activated_machines` on those rows rather than adding code for a case that does not exist. **This simplification expires the moment the beta ships** — after that, any tightening of a licence limit needs a migration path.
- **Default batch size:** the cap is the ceiling, not the default. **Set to 25** (was 10) — agreed with the reviewer's recommendation; with U1 streaming the first titles land in ~5s either way, and a shorter default feels faster. Desktop `27394f3` (`src/index.html` slider value 25 + labels 1/50/100/200; `setupSlider` max 50/100/200).

**T2. Sales page — all three tiers *(§6.4b item 5)* — ⚠️ PARTLY DONE (`6d4e3b7`). GATE 5 REOPENED on review: four fixes landed, one false bullet survived, two differentiators unsold.**

> **Three things left — roughly twenty minutes:**
>
> 1. ❌ **Pro's "Full appeal breakdowns & scores" is FALSE on desktop** — `desktop.html:440` (card) **and `:517` (FAQ)**, so it appears twice. Not gated: `engine.rs` sets `breakdown: Some(...)` on every result regardless of tier, and nothing in `app.js` gates its display. **Core already has it.** Root cause is worth understanding because it is the same one as the original Studio errors: the *web* app genuinely does gate this (`titleforge/app.js:514` "Pro unlocks … full appeal breakdowns", `:2386` PRO badges) and the framing was copied onto the desktop card. **§3 of this file already warns that desktop and web diverge deliberately — that warning applies to sales copy, not just code.** Either drop the bullet or gate it on desktop; do not silently gate it just to make the claim true.
> 2. ❌ **Projects are sold nowhere.** Gated to Pro/Studio by four `== "core"` checks (`get_projects`, `create_project`, `add_to_project`, `update_title_notes`) and on no card. A real differentiator you are not selling.
> 3. ❌ **Studio's 5 machine activations are missing from the card.** Core shows 1 (`:423`), Pro shows 3 (`:441`), Studio shows none — though the FAQ (`:517`) does say 5. Card and FAQ disagree, and the tier with the most looks like it has none.
>
> Minor: `supabase-setup.sql:247` still says "3-device limit" — stale since the limit became tier-aware.
>
> **Closing test, apply to every bullet on every card:** name the line of code that enforces it. No line, not a differentiator — it is a promise, and promises belong with the commercial-licence / support / upgrade group, not the feature list.

**What T2 did fix, and these were real:** U2 fixed only Studio, where 4 of 8 bullets were false. **Core and Pro have never had the same audit.** Same method: every bullet checked against the gate that enforces it. Findings fixed: Core "up to 25 titles" → **50**; Core "3 machine activations" → **1**; Core "Lifetime version updates" → **"Lifetime minor updates within your version"** (otherwise Studio's "free major version upgrade" is not a differentiator); Pro removed the false **"Priority update delivery"** bullet (no tier-aware updater exists — `tauri-plugin-updater` is uniform across tiers, `lib.rs:1238`/`app.js:471`). FAQ matched: batch sizes 50/100/200, machines 1/3/5, free major-version upgrade is Studio-only. Added the machine counts. Nothing on the page claims code that does not enforce it. Prices, `pricing_config`, `data-price` untouched. **(T3/T4 are now REAL code and on the page — see below.)**

**T3. Brand voice profiles → Pro — ✅ DONE 2026-08-07 (`913042f`).** `AI-WORK-BRIEF.md:257`. New `brand_voice_profiles` table; `FineTune.brand_voice` + parse; `engine.rs` prefers the user's exemplar titles over curated RAG in the few-shot slot (capped at 4). Commands `save_brand_voice`/`get_brand_voices`/`set_active_brand_voice`/`delete_brand_voice`, Pro-gated. Frontend Brand Voice settings card; active profile injected as `brandVoice`. Test added (`finetune_parses_brand_voice_exemplars`). On the Pro sales card + FAQ.

**T4. Bulk CSV mode → Studio — ✅ DONE 2026-08-07 (`913042f`).** `AI-WORK-BRIEF.md:258`. `bulk_generate_csv` command: parse keywords one-per-line, loop the offline engine (Studio cap 200/keyword, 500 bound, honors cancel), return `keyword,title,score` CSV. Studio-gated. Frontend Studio-only **Bulk** dashboard tab with Download/Copy. On the Studio sales card + FAQ.
