...
2. Auto-updater has completed a real install → update cycle — ✅ **CLOSED 2026-08-07, validated against the GitHub endpoint.** Beta.6 installed in Sandbox and self-updated to beta.7 from `releases/latest/download/updates.json` (owner-verified, app reports `1.0.0-beta.7`). The cycle surfaced + fixed a real defect: tagging without bumping `Cargo.toml`/`tauri.conf.json`/`package.json` named assets `1.0.0-beta.5` under a beta.6 tag and pointed updates.json URLs at nonexistent files (silent 404). Bump all three in lockstep with the tag. Both releases independently audited PASS. Root CONTEXT §6.4b has the full note.
3. First-launch download verified on a clean machine, on a real connection
4. Studio batch time is honest — **✅ CLOSED 2026-08-07 by the tier decision.** Studio re-take DONE (2 runs, `studio-batch-run1/2.csv`): 199/200 and 200/200 yield (100%), duplicate:QC ~200:1, 26-31 min. The 2026-08-07 tier decision keeps the cap at 200 and U1 streaming makes it watchable, so the gate is closed. (Was: "STILL OPEN, hardest gate" — superseded.)
5. Every sales-page claim matches measured reality (engine name, batch sizes, offline quality) — **✅ CLOSED 2026-08-07 by T2** (`6d4e3b7`). Core and Pro audited like Studio; Core "up to 25 titles" → 50, Core "3 machine activations" → 1, Core "lifetime version updates" → "lifetime minor updates within your version"; Pro's false "Priority update delivery" bullet removed (no tier-aware updater). FAQ matched to 50/100/200 and 1/3/5. Every desktop bullet maps to a code-enforced gate.
