...
**Current desktop next action:** D1 (fill budget) landed — flat 2× + early exit + no noise sort in `engine.rs` (commit `7017702`); order-preserving final dedup (commit `839a705`); Core-25 through `engine::generate` PASSES post-D1 (25/25 unique in 181.9s, commit 839a705 evidence). after-D1 `category_fit` shows no regression (96% fire, range 7.06, product 16/16; CSVs `category-fit-after-d1.csv`, `yield-curve-after-d1.csv`). Studio harness rebuilt in `yield_curve.rs` shape (`bc485f0`) — two Studio runs (post-D1, ~52 min each) are next, then the Studio cap decision to the owner. ⛔ The beta tag + live in-app update against the GitHub endpoint is also still deferred to the owner. Do not restart rejected ranker work, Qwen2.5-3B, multi-constraint prompt experiments, colon caps, grammar work, or desktop dual-provider pairing.
