...
- **Seed data:** 1,300 templates (~81 per category × 16), 889 word pool entries across 8 pools, 2,623 curated titles across 16 categories with 9 tones each
...
| `seed-data.json` | 1.0MB | Generated by DeepSeek V4 Pro (original 796 curated titles + all templates/pools, ~$3) + DeepSeek V4 Flash (1,827 additional tone-tagged curated titles, ~$12): 1,300 templates (~81/category), 889 word pool entries across 8 pools, 2,623 curated titles across 16 categories (tone-tagged: normal + 8 distinct tones) |
...
- Desktop app: Tauri v2 shell, Rust IPC (19 commands), SQLite schema (8 tables), offline engine, seed data imported (2,623 curated titles with real tone metadata), AI integration (4 providers), license system, activation screen, XOR API key obfuscation
...
- **Seed data generated by AI:** DeepSeek V4 Pro (initial ~$3 pass) + DeepSeek V4 Flash (tone-tagged expansion, ~$12), ~$15 total cost, 1,300 templates + 889 word pool entries + 2,623 curated titles (with real tone metadata — 9 distinct tones per category)
...
| `seed-data.json` | 1.0MB | JSON | 1,300 templates + 889 words + 2,623 curated titles |
